...
import com.pc.service.mapper.DtsAccountTraceMapper;
import com.pc.service.mapper.DtsUserAccountMapper;
import com.pc.service.mapper.DtsUserMapper;
import com.pc.service.mapper.ex.AccountMapperEx;
...
public class DtsAccountService {
    ...
    public boolean settlementPreviousAgency(Integer userId) {
        ...
        if (toSettleMoney == null || toSettleMoney.compareTo(new BigDecimal("0")) == 0) {// 如果该用户未产生订单
            log.info("用户 userId:{} 不存在未结算的订单,给其代理人结算佣金结束!");
            return true;
        }
        // 获取当前用户的代理
