...
import com.pc.service.domain.DtsCart;
import com.pc.service.domain.DtsCoupon;
import com.pc.service.domain.DtsCouponUser;
import com.pc.service.domain.DtsGrouponRules;
import com.pc.service.service.*;
import com.pc.service.util.CouponConstant;
...
import com.wx.mall.dao.CouponVo;
...
public class WxCouponController {
    ...
    public Object list(@RequestParam(defaultValue = "1") Integer page, @RequestParam(defaultValue = "10") Integer size,
        ...
        log.info("【请求开始】获取优惠券列表,请求参数,page:{},size:{}", page, size);
        ...
        log.info("【请求结束】获取优惠券列表,响应内容:{}", JSONObject.toJSONString(data));
    ...
    public Object mylist(@LoginUser Integer userId, @NotNull Short status,
                         ...
                         @Sort @RequestParam(defaultValue = "add_time") String sort,
                         @Order @RequestParam(defaultValue = "desc") String order) {
        log.info("【请求开始】个人优惠券列表,请求参数,userId:{},page:{},size:{}", userId, page, size);

        if (userId == null) {
            log.error("个人优惠券列表失败:用户未登录！！！");
        ...
        log.info("【请求结束】个人优惠券列表,响应内容:{}", JSONObject.toJSONString(data));
        return ResponseUtil.ok(data);
    }
    ...
    public Object getUserCoupon(@LoginUser Integer userId) {
        log.info("【请求开始】个人可领取优惠券列表,请求参数,userId:{}", userId);

        if (userId == null) {
            log.error("个人可领取优惠券列表:用户未登录！！！");
            return ResponseUtil.unlogin();
        }
        ...
        log.info("【请求结束】个人可领取优惠券列表,响应内容:{}", JSONObject.toJSONString(data));
        return ResponseUtil.ok(data);
    }
    ...
    public Object selectlist(@LoginUser Integer userId, Integer cartId, Integer grouponRulesId) {
        log.info("【请求开始】当前购物车下单商品订单可用优惠券,请求参数,userId:{},cartId:{},grouponRulesId:{}", userId, cartId,
                grouponRulesId);

        if (userId == null) {
            log.error("当前购物车下单商品订单可用优惠券:用户未登录！！！");
            return ResponseUtil.unlogin();
        }
        ...
        log.info("【请求结束】当前购物车下单商品订单可用优惠券,响应内容:{}", JSONObject.toJSONString(couponVoList));
    ...
    public Object receive(@LoginUser Integer userId, @RequestBody String body) {
        log.info("【请求开始】优惠券领取,请求参数,userId:{},body:{}", userId, body);

        if (userId == null) {
            log.error("优惠券领取:用户未登录！！！");
            return ResponseUtil.unlogin();
        }
        ...
        if ((total != 0) && (totalCoupons >= total)) {
            log.error("优惠券领取出错:{}", WxResponseCode.COUPON_EXCEED_LIMIT.desc());
        ...
        if (type.equals(CouponConstant.TYPE_REGISTER) || type.equals(CouponConstant.TYPE_CODE)
            ...
            log.error("优惠券领取出错:{}", WxResponseCode.COUPON_CODE_INVALID.desc());
        ...
        if (status.equals(CouponConstant.STATUS_OUT)) {
            ...
            log.error("优惠券领取出错:{}", WxResponseCode.COUPON_EXPIRED.desc());
    ...
    public Object receiveAll(@LoginUser Integer userId) {
        log.info("【请求开始】一键领取优惠券，请求参数：userId:{}", userId);

        if (userId == null) {
            log.error("一键领取优惠券:用户未登录！！！");
            return ResponseUtil.unlogin();
        }
    ...
    public Object exchange(@LoginUser Integer userId, @RequestBody String body) {
        log.info("【请求开始】优惠券兑换，请求参数：userId:{},Body:{}", userId, body);

        if (userId == null) {
            log.error("优惠券兑换:用户未登录！！！");
            return ResponseUtil.unlogin();
        }
        ...
        if (coupon == null) {
            log.error("优惠券兑换出错:{}", WxResponseCode.COUPON_CODE_INVALID.desc());
            return WxResponseUtil.fail(WxResponseCode.COUPON_CODE_INVALID);
        }
        ...
        Integer totalCoupons = couponUserService.countCoupon(couponId);
        if ((total != 0) && (totalCoupons >= total)) {
            log.error("优惠券兑换出错:{}", WxResponseCode.COUPON_EXCEED_LIMIT.desc());
            return WxResponseUtil.fail(WxResponseCode.COUPON_EXCEED_LIMIT);
        }
        ...
        if (type.equals(CouponConstant.TYPE_REGISTER) || type.equals(CouponConstant.TYPE_COMMON)
            ...
            log.error("优惠券兑换出错:{}", WxResponseCode.COUPON_NOT_CHANGE.desc());
        ...
        if (status.equals(CouponConstant.STATUS_OUT)) {
            ...
            log.error("优惠券兑换出错:{}", WxResponseCode.COUPON_EXPIRED.desc());
            return WxResponseUtil.fail(WxResponseCode.COUPON_EXPIRED);
        }
