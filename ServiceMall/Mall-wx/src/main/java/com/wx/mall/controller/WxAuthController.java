...
import com.pc.service.service.CouponAssignService;
import com.pc.service.service.DtsUserService;
import com.wx.core.captcha.CaptchaCodeManager;
import com.wx.core.consts.CommConsts;
import com.wx.core.notify.NotifyService;
import com.wx.core.notify.NotifyType;
import com.wx.core.notify.SmsResult;
import com.wx.core.type.UserTypeEnum;
...
import com.wx.core.util.bcrypt.BCryptPasswordEncoder;
import com.wx.mall.annotation.LoginUser;
import com.wx.mall.dao.UserInfo;
import com.wx.mall.dao.UserToken;
import com.wx.mall.dao.WxLoginInfo;
import com.wx.mall.service.UserTokenManager;
import com.wx.mall.util.IpUtil;
...
import static com.wx.mall.util.WxResponseCode.*;
...
public class WxAuthController {
    ...
    public Object login(@RequestBody String body, HttpServletRequest request) {
        log.info("【请求开始】账户登录,请求参数,body:{}", body);
        ...
        DtsUser user = null;
        if (userList.size() > 1) {
            log.error("账户登录 出现多个同名用户错误,用户名:{},用户数量:{}", username, userList.size());
            return ResponseUtil.serious();
        } else if (userList.size() == 0) {
            log.error("账户登录 用户尚未存在,用户名:{}", username);
        ...
        if (!encoder.matches(password, user.getPassword())) {
            log.error("账户登录 ,错误密码：{},{}", password, AUTH_INVALID_ACCOUNT.desc());// 错误的密码打印到日志中作为提示也无妨
        ...
        try {
            ...
            log.error("账户登录：设置用户指定信息出错：" + e.getMessage());
            e.printStackTrace();
        }
        ...
        try {
            ...
            log.error("账户登录失败,生成token失败：{}", user.getId());
            e.printStackTrace();
        ...
        log.info("【请求结束】账户登录,响应结果:{}", JSONObject.toJSONString(result));
    ...
    public Object registerCaptcha(@RequestBody String body) {
        log.info("【请求开始】请求验证码,请求参数，body:{}", body);
        ...
        if (!notifyService.isSmsEnable()) {
            log.error("请求验证码出错:{}", AUTH_CAPTCHA_UNSUPPORT.desc());
        ...
        if (smsResult != null) {
            log.info("*****腾讯云短信发送->请求验证码，短信发送结果：{}", JSONObject.toJSONString(smsResult));
        }

        boolean successful = CaptchaCodeManager.addToCache(phoneNumber, code, 1);
        if (!successful) {
            log.error("请求验证码出错:{}", AUTH_CAPTCHA_FREQUENCY.desc());
            return WxResponseUtil.fail(AUTH_CAPTCHA_FREQUENCY);
        }

        log.info("【请求结束】请求验证码成功");
    ...
    public Object register(@RequestBody String body, HttpServletRequest request) {
        log.info("【请求开始】账号注册,请求参数，body:{}", body);
        ...
        List<DtsUser> userList = userService.queryByUsername(username);
        if (userList.size() > 0) {
            log.error("请求账号注册出错:{}", AUTH_NAME_REGISTERED.desc());
            return WxResponseUtil.fail(AUTH_NAME_REGISTERED);
        }

        userList = userService.queryByMobile(mobile);
        if (userList.size() > 0) {
            log.error("请求账号注册出错:{}", AUTH_MOBILE_REGISTERED.desc());
            return WxResponseUtil.fail(AUTH_MOBILE_REGISTERED);
        }
        if (!RegexUtil.isMobileExact(mobile)) {
            log.error("请求账号注册出错:{}", AUTH_INVALID_MOBILE.desc());
            return WxResponseUtil.fail(AUTH_INVALID_MOBILE);
        }
        // 判断验证码是否正确
        String cacheCode = CaptchaCodeManager.getCachedCaptcha(mobile);
        if (cacheCode == null || cacheCode.isEmpty() || !cacheCode.equals(code)) {
            log.error("请求账号注册出错:{}", AUTH_CAPTCHA_UNMATCH.desc());
            return WxResponseUtil.fail(AUTH_CAPTCHA_UNMATCH);
        }

        String openId = "123";  //使用默认openid用于测试
        /*try {
            WxMaJscode2SessionResult result = this.wxService.getUserService().getSessionInfo(wxCode);
            openId = result.getOpenid();
        } catch (Exception e) {
            log.error("请求账号注册出错:{}", AUTH_OPENID_UNACCESS.desc());
            e.printStackTrace();
        ...
        if (userList.size() == 1) {
            ...
            if (!checkUsername.equals(openId) || !checkPassword.equals(openId)) {
                log.error("请求账号注册出错:{}", AUTH_OPENID_BINDED.desc());
        ...
        try {
            ...
            log.error("账号注册失败,给新用户发送注册优惠券失败：{}", user.getId());
            e.printStackTrace();
            return ResponseUtil.fail();
        }
        ...
        try {
            ...
            log.error("账号注册失败,生成token失败：{}", user.getId());
            e.printStackTrace();
            return ResponseUtil.fail();
        }
        ...
        log.info("【请求结束】账号注册,响应结果:{}", JSONObject.toJSONString(result));
        return ResponseUtil.ok(result);
    }
    ...
    public Object reset(@RequestBody String body, HttpServletRequest request) {
        log.info("【请求开始】账号密码重置,请求参数，body:{}", body);
        ...
        String cacheCode = CaptchaCodeManager.getCachedCaptcha(mobile);
        if (cacheCode == null || cacheCode.isEmpty() || !cacheCode.equals(code)) {
            log.error("账号密码重置出错:{}", AUTH_CAPTCHA_UNMATCH.desc());
            return WxResponseUtil.fail(AUTH_CAPTCHA_UNMATCH);
        }
        ...
        DtsUser user = null;
        if (userList.size() > 1) {
            log.error("账号密码重置出错,账户不唯一,查询手机号:{}", mobile);
            return ResponseUtil.serious();
        } else if (userList.size() == 0) {
            log.error("账号密码重置出错,账户不存在,查询手机号:{},{}", mobile, AUTH_MOBILE_UNREGISTERED.desc());
        ...
        if (userService.updateById(user) == 0) {
            log.error("账号密码重置更新用户信息出错,id：{}", user.getId());
            return ResponseUtil.updatedDataFailed();
        }

        log.info("【请求结束】账号密码重置成功!");
        return ResponseUtil.ok();
    }
