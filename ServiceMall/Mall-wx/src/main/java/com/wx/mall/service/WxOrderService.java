...
import com.pc.service.domain.*;
import com.pc.service.service.*;
import com.pc.service.util.CouponUserConstant;
import com.pc.service.util.OrderHandleOption;
import com.pc.service.util.OrderUtil;
import com.wx.core.consts.CommConsts;
import com.wx.core.express.ExpressService;
import com.wx.core.express.dao.ExpressInfo;
import com.wx.core.express.dao.Traces;
import com.wx.core.notify.NotifyService;
import com.wx.core.notify.NotifyType;
import com.wx.core.qcode.QCodeService;
import com.wx.core.system.SystemConfig;
import com.wx.core.util.DateTimeUtil;
import com.wx.core.util.JacksonUtil;
import com.wx.core.util.ResponseUtil;
import com.wx.mall.dao.BrandCartGoods;
import com.wx.mall.dao.BrandOrderGoods;
...
public class WxOrderService {
    ...
    public Object list(Integer userId, Integer showType, Integer page, Integer size) {
        ...
        log.info("【请求结束】获取订单列表,响应结果:{}", JSONObject.toJSONString(result));
    ...
    public Object detail(Integer userId, Integer orderId) {
        ...
        log.info("【请求结束】获取订单详情,响应结果:{}", JSONObject.toJSONString(result));
        return ResponseUtil.ok(result);
    ...
    public Object submit(Integer userId, String body) {
        ...
        if (grouponRulesId != null && grouponRulesId > 0) {
            ...
            if (grouponRulesService.isExpired(rules)) {
                log.error("提交订单详情失败：{}", GROUPON_EXPIRED.desc());
        ...
        log.info("【请求结束】提交订单,响应结果:{}", JSONObject.toJSONString(data));
    ...
    public Object prepay(Integer userId, String body, HttpServletRequest request) {
        ...
        if (!handleOption.isPay()) {
            log.error("付款订单的预支付会话标识失败：{}", WxResponseCode.ORDER_REPAY_OPERATION.desc());
    ...
    public Object dtsPayNotify(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (groupon != null) {
            ...
            if (grouponService.updateById(groupon) == 0) {
                log.error("微信付款成功或失败回调失败：{}", "更新团购数据已失败!");
        ...
        if (SystemConfig.isMultiOrderModel() && brandOrderGoodsList.size() > 1) {// 如果是多店铺模式，且这个订单确实含有多个店铺商品
            log.info("需要进行拆单：主订单编号：【" + order.getId() + "】 支付编号【" + payId + "】");
            ...
            if (payTotalFee.compareTo(checkActualPrice) < 0 || errorPrice
                ...
                log.warn("拆单过程存在误差已超出千分之五，请联系技术人员核查比对：订单编号：【" + orderSn + "】");
            }
