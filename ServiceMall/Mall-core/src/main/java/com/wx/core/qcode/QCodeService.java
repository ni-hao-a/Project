...
public class QCodeService {
	...
	public String createShareUserImage(Integer userId) {
		...
		try {
			...
			log.error("创建代理用户推荐共享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
		} catch (IOException e) {
			log.error("创建代理用户推荐共享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
	...
	public String createShareTopicImage(Integer topicId, String picUrl, String subtitle, BigDecimal price) {
		...
		try {
			...
			log.info("创建活动主题分享图 URL:{}",url);
			return url;
		} catch (WxErrorException e) {
			log.error("创建活动主题分享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
		} catch (Exception e) {
			log.error("创建活动主题分享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
	...
	public String createBrandImage(Integer shareUserId,Integer brandId, String picUrl, String name, String defaultCategory) {
		...
		try {
			...
			log.info("创建品牌商铺主题分享图 URL:{}",url);
			return url;
		} catch (WxErrorException e) {
			log.error("创建品牌商铺分享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
		} catch (Exception e) {
			log.error("创建品牌商铺分享二维码 错误:{}",e.getMessage());
			e.printStackTrace();
