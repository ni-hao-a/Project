package com.pc.seckill.service.impl;

import com.pc.seckill.common.aop.Servicelock;
...
import com.pc.seckill.mapper.SecKillMapper;
import com.pc.seckill.mapper.SuccessSecKillMapper;
...
public class SeckillServiceImpl implements ISeckillService {
    ...
    public Long getSeckillCount(long seckillId) {
        int count = successSecKillMapper.getSecKillCount(seckillId);
    ...
    public void deleteSeckill(long seckillId) {
        successSecKillMapper.delResult(seckillId);
    ...
    public Result startSeckil(long seckillId, long userId) {
        // 校验商品库存
        Seckill seckill = secKillMapper.getSecKillCount(seckillId);
        int number = seckill.getNumber();
        if (number > 0) {
            // 扣库存
            secKillMapper.updateSecKillDed(seckillId);
            //创建订单
            ...
            successSecKillMapper.saveResult(killed);
            /**
    ...
    public Result startSeckilDBPCC_TWO(long seckillId, long userId) {
         ...
         */
        int count = secKillMapper.updateStockAndDed(seckillId);
        if (count > 0) {
            SuccessKilled killed = new SuccessKilled();
            killed.setSeckillId(seckillId);
            killed.setUserId(userId);
    ...
    public Result startSeckilDBOCC(long seckillId, long userId, long number) {
        Seckill kill = secKillMapper.getSecKillCount(seckillId);
        /**
         * 剩余的数量应该要大于等于秒杀的数量
         */
        if (kill.getNumber() >= number) {
            //乐观锁
            int count = secKillMapper.updateStock(seckillId, kill.getVersion());
            if (count > 0) {
                SuccessKilled killed = new SuccessKilled();
                killed.setSeckillId(seckillId);
                killed.setUserId(userId);
