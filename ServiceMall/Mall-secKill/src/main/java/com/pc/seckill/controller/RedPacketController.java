...
import com.pc.seckill.common.utils.DoubleUtil;
import com.pc.seckill.queue.delay.jvm.RedPacketMessage;
import com.pc.seckill.queue.delay.jvm.RedPacketQueue;
import com.pc.seckill.service.IRedPacketService;
...
public class RedPacketController {
    ...
    public Result startThree(long redPacketId) {
        ...
        try {
            latch.await();
            Integer restMoney = Integer.parseInt(redisUtil.getValue(redPacketId + "-money").toString());
            log.info("剩余金额：{}", DoubleUtil.divide(Double.parseDouble(restMoney.toString()), (double) 100));
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
        return Result.ok();
